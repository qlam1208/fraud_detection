...
class FraudDetectionInference:
    ...
    def add_features(self, df):
        ...
        # Feature mới thêm so với repo: Geographic feature từ location
        df = df.withColumn("is_foreign", when(col("location") != 'VN', 1).otherwise(0))
    ...
    def run_inference(self):
        ...
        def predict_udf(
            ...
            """
            # Create input DataFrame from feature columns matching exactly the training script
            ...
            threshold = 0.0  # Tuned based on precision/recall requirements
        ...
        (fraud_predictions.selectExpr(
         ...
         .option("topic", self.config['kafka'].get('output_topic', 'fraud_predictions'))  # Output topic for fraud alerts
         ...
         .option("checkpointLocation", self.config['spark'].get('checkpoint_location', 'checkpoints/checkpoint'))  # For fault tolerance and recovery
...
if __name__ == "__main__":
    ...
    inference = FraudDetectionInference("/app/config/config.yaml")
