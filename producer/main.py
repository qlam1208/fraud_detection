...
TRANSACTION_SCHEMA = {
    ...
    "properties": {
        ...
        "amount": {"type": "number", "minimum": 10000, "maximum": 1000000000},
...
class TransactionProducer:
    def __init__(self):
        ...
        self.high_risk_merchants = ["CryptoExchangeVN",
                                    "DigitalTopup24",
                                    "OnlineGiftCard",
                                    "VirtualGoldStore",
                                    "QuickTransfer"]
    ...
    def generate_transaction(self) -> Optional[Dict[str, Any]]:
        ...
        transaction = {
            ...
            "amount": random.randint(1, 50000) * 10000,  # Reduced max to 1 to 500tr
            "currency": "VND",
            "merchant": random.choice(VN_MERCHANTS) if random.random() < 0.7 else fake.company(),
            "timestamp": (datetime.now(timezone.utc) +
                         timedelta(seconds=random.randint(-300, 300))).isoformat(),
            "location": "VN" if random.random() < 0.95 else fake.country_code(),
        ...
        if user_id in self.compromised_users and amount > 5000000:
            if random.random() < 0.3:  # 30% chance of fraud in compromised accounts
                is_fraud = 1
                transaction['amount'] = random.randint(1500, 50000) * 10000  # Typical takeover amounts
                transaction['merchant'] = random.choice(self.high_risk_merchants)

        # Pattern 2: Card testing (0.45% of total)
        if not is_fraud and amount < 20000:
            # Simulate rapid small transactions (every 10th user in testing mode)
            if user_id % 1000 == 0 and random.random() < 0.25:
                is_fraud = 1
                transaction['amount'] = round(random.uniform(1000, 20000), -3)
                transaction['location'] = 'VN'  # Consistent testing location
        ...
        if not is_fraud and merchant in self.high_risk_merchants:
            if amount > 300 and random.random() < 0.15:
                is_fraud = 1
                transaction['amount'] = random.randint(2000, 50000) * 10000
        ...
        if not is_fraud:
            ...
            if user_id % 500 == 0 and random.random() < 0.1:
                is_fraud = 1
                transaction['location'] = random.choice(['KH', 'TH', 'SG', 'AE'])  # High-risk countries

        # Baseline random fraud (0.1-0.3%)
        if not is_fraud and random.random() < 0.002:
            is_fraud = 1
            transaction['amount'] = round(random.uniform(1_000_000, 50_000_000), -4)
